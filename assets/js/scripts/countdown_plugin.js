...
export function countdown_init() {
		'use strict';
		$.fn.countdown = function (options) {
			return $.fn.countdown.begin(this, $.extend({
				...
				labels: false, // If false days hours seconds and monutes labels will not be created
		...
		$.fn.countdown.begin = function (parent, settings) {
			...
			if (!settings.init) {
				...
				$.each(timespan, function (k, v) {
					...
					container = $('<div/>').addClass('col-sm dateblock').attr('id', k);
